...
class TestTestrunViews(TestCase):
    ...
    def test_input_views(self):
        ...
        with fake_time(datetime(2011, 8, 5, tzinfo=UTC)):
            show_output = self.client.get(reverse("get_testrun_input", kwargs=kwargs), headers={"x-requested-with": "XMLHttpRequest"})
            self.assertContains(show_output, "9 9")
            self.assertContains(show_output, "Input")

            download_response = self.client.get(reverse("download_testrun_input", kwargs=kwargs))

            self.assertContains(download_response, "9 9\n")
            self.assertTrue(download_response["Content-Disposition"].startswith("attachment"))
            self.assertIn('filename="input.in"', download_response["Content-Disposition"])
    ...
    def test_output_views(self):
        ...
        with fake_time(datetime(2011, 8, 5, tzinfo=UTC)):
            show_output = self.client.get(reverse("get_testrun_output", kwargs=kwargs), headers={"x-requested-with": "XMLHttpRequest"})
            self.assertContains(show_output, "18")
            self.assertContains(show_output, "Output")

            download_response = self.client.get(reverse("download_testrun_output", kwargs=kwargs))

            self.assertContains(download_response, "18\n")
            self.assertTrue(download_response["Content-Disposition"].startswith("attachment"))
            self.assertIn('filename="output.out"', download_response["Content-Disposition"])
    ...
    def test_archive_submission(self):
        ...
        for bad_archive in ["over_limit.zip", "two_files.zip", "evil.zip"]:
            ...
            with open(filename, "rb") as input_file:
                ...
                response = self.client.post(url, data, follow=True)
                self.assertEqual(response.status_code, 200)
                self.assertEqual(Submission.objects.filter(kind="TESTRUN").count(), testruns_before)
        ...
        with open(os.path.join(base_dir, "single_file.zip"), "rb") as input_file:
            ...
            response = self.client.post(url, data, follow=True)
            self.assertEqual(response.status_code, 200)
            self.assertEqual(Submission.objects.filter(kind="TESTRUN").count(), testruns_before + 1)
            submission = TestRunProgramSubmission.objects.latest("pk")
            self.assertEqual(submission.kind, "TESTRUN")
            self.assertEqual(submission.source_file.read().strip(), b"a")
...
class TestTestRunsLimit(TestCase, TestRunTestCase, SubmitFileMixin):
    ...
    def test_test_run_limit_should_be_respected(self):
        ...
        self.assertNotRegex(first_test_run_response.content.decode("utf-8"), "limit.*exceeded")
        self.assertRegex(second_test_run_response.content.decode("utf-8"), "limit.*exceeded")
    ...
    def test_test_run_limit_should_be_independent_from_submission_limit(self):
        ...
        self.assertRegex(second_test_run_response.content.decode("utf-8"), "limit.*exceeded")
        self.assertNotRegex(second_normal_response.content.decode("utf-8"), "limit.*exceeded")
    ...
    def test_zero_test_run_limit_should_mean_unlimited_test_runs(self):
        ...
        self.assertNotRegex(first_test_run_response.content.decode("utf-8"), "limit.*exceeded")
        self.assertNotRegex(second_test_run_response.content.decode("utf-8"), "limit.*exceeded")
