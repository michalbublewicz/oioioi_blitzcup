...
class TestRunProblemControllerMixin:
    ...
    def fill_evaluation_environ(self, environ, submission, **kwargs):
        ...
        environ["error_handlers"].append(("delete_output", "oioioi.testrun.handlers.delete_output"))
...
class TestRunContestControllerMixin:
        ...
        def validate_zip(file):
            if is_zipfile(file):
                ...
                if archive.extracted_size() > self.get_testrun_unzipped_input_limit():
                    raise ValidationError(_("Uncompressed archive is too big to be considered safe."))
    ...
    def _render_testrun_report(self, request, report, testrun_report, template="testrun/report.html"):
        score_report = ScoreReport.objects.get(submission_report=report)
    ...
    def get_test_run_time_limit(self, problem_instance):
        return problem_instance.problem.controller.get_test_run_time_limit(problem_instance)

    def get_test_run_memory_limit(self, problem_instance):
        return problem_instance.problem.controller.get_test_run_memory_limit(problem_instance)
