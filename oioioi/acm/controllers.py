...
class ACMContestController(ProgrammingContestController):
    ...
    def get_submission_relative_time(self, submission):
        ...
        rtimes = rounds_times(DummyRequest(submission.user or AnonymousUser()), self.contest)
    ...
    def update_user_result_for_problem(self, result):
        ...
        if last_submission:
            result.submission_report = last_submission.submissionreport_set.get(status="ACTIVE", kind="FULL")

            if last_submission.status == "OK":
                # FIXME: May not ignore submissions with admin-hacked same-date
                submissions.filter(date__gt=last_submission.date).update(status="IGN", score=None)
        else:
...
class ACMRankingController(DefaultRankingController):
    ...
    def _get_csv_row(self, key, row):
        ...
        line += [r.score.csv_repr() if r and r.score is not None else "" for r in row["results"]]
    ...
    def _get_old_results(self, freeze_time, pis, users):
        ...
        results = []
        for user, user_submissions in itertools.groupby(submissions, attrgetter("user")):
            for pi, user_pi_submissions in itertools.groupby(user_submissions, attrgetter("problem_instance")):
    ...
    def serialize_ranking(self, key):
        ...
        pis = list(ProblemInstance.objects.filter(round__in=rounds).select_related("problem").prefetch_related("round"))
        ...
        frozen = False
        for round, freeze_time in zip(rounds, freeze_times, strict=False):
            rpis = rtopis[round]
            rtimes = ccontroller.get_round_times(None, round)
            now = timezone.now()
            if freeze_time is None or self.is_admin_key(key) or rtimes.results_visible(now) or now <= freeze_time:
                results += (
                    UserResultForProblem.objects.filter(problem_instance__in=rpis, user__in=users)
...
class NotificationsMixinForACMContestController:
    ...
    def get_notification_message_submission_judged(self, submission):
        if submission.score is not None and submission.score.accepted:
            return gettext_noop("Your submission for task %(short_name)s is accepted. Congratulations!")
        else:
            return gettext_noop("Your submission for task %(short_name)s is not accepted.")
