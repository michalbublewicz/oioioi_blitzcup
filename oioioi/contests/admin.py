...
class RoundInline(admin.StackedInline):
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if request.contest is not None and request.contest.controller.separate_public_results():
...
class ContestAdmin(admin.ModelAdmin):
    ...
    def get_fields(self, request, obj=None):
        fields = ["name", "id", "school_year", "controller_name", "default_submissions_limit", "contact_email"]
...
class BaseContestAdmin(admin.MixinsAdmin):
    ...
    def _mixins_for_instance(self, request, instance=None):
        if instance:
            ...
            return controller.mixins_for_admin() + controller.registration_controller().mixins_for_admin()
...
contest_admin_menu_registry.register(
    "contest_change",
    _("Settings"),
    lambda request: reverse("oioioiadmin:contests_contest_change", args=(quote(request.contest.id),)),
    order=20,
)
...
class ProblemInstanceAdmin(admin.ModelAdmin):
    ...
    def attach_problems_to_another_contest(self, request, queryset):
        return redirect(self._attach_problem_ids_to_url(queryset, "reattach_problem_contest_list"))

    @action(description=_("Assign problems to a round"))
    def assign_problems_to_a_round(self, request, queryset):
        return redirect(self._attach_problem_ids_to_url(queryset, "assign_problems_to_a_round"))
    ...
    def _replace_statement_href(self, instance):
        return reverse("problem_site", args=(instance.problem.problemsite.url_key,)) + "?" + urllib.parse.urlencode({"key": "replace_problem_statement"})

    def _package_manage_href(self, instance):
        return reverse("problem_site", args=(instance.problem.problemsite.url_key,)) + "?" + urllib.parse.urlencode({"key": "manage_files_problem_package"})
...
class SubmissionAdmin(admin.ModelAdmin):
    ...
    def rejudge_view(self, request):
        ...
        for sub in submissions.values():
            if not SubmissionReport.objects.filter(submission=sub, status="ACTIVE").exists():
        ...
        if all_reports_exist or rejudge_type == "FULL":
            ...
            self.message_user(
                request,
                _("Cannot rejudge submissions due to lack of active reports for one or more submissions"),
            )
    ...
    def problem_instance_display(self, instance):
        if instance.kind != "NORMAL":
            ...
            return f"{instance.problem_instance.problem.legacy_name} ({instance.problem_instance.get_short_name_display()})"
    ...
    def rejudge_action(self, request, queryset):
        ...
        self.message_user(
            request,
            _("You have selected %(sub_count)d submission(s) from %(pis_count)d problem(s)") % {"sub_count": sub_count, "pis_count": pis_count},
        )
    ...
    def get_queryset(self, request):
        ...
        queryset = queryset.annotate(score_with_nulls_smallest=Coalesce("score", Value("")))
        return queryset
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        return redirect("submission", contest_id=_contest_id, submission_id=unquote(object_id))
...
admin.system_admin_menu_registry.register(
    ...
    _("All submissions"),
    lambda request: reverse("oioioiadmin:contests_submission_changelist", kwargs={"contest_id": None}),
    order=50,
)
...
class RoundTimeRoundListFilter(SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        return Round.objects.filter(id__in=qs.values_list("round")).values_list("id", "name")
...
class ContestPermissionAdmin(admin.ModelAdmin):
    ...
    def has_change_permission(self, request, obj=None):
        ...
        return is_contest_owner(request) and request.contest == obj.contest and obj.permission != "contests.contest_owner"
